...
function handleFile(card, file) {
  ...
  reader.onload = () => {
    ...
    status.textContent = "Icon loaded. Save the workspace to apply changes.";
  };
...
function bindIconRow(card) {
  ...
  clearBtn?.addEventListener("click", () => {
    setPreview(card, "");
    status.textContent = "Custom icon cleared. Save the workspace to apply changes.";
  });
...
saveBtn?.addEventListener("click", async () => {
  ...
  status.textContent = `Saved district workspace with ${normalized.length} tab${normalized.length === 1 ? "" : "s"}. Opening Dock…`;
  setTimeout(() => {
