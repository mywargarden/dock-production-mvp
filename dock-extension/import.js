...
async function importWorkspace(){
  ...
  const tabs = Array.isArray(workspace.tabs) ? workspace.tabs.map((tab) => {
    ...
    return {
      ...
      // Preserve the shared preview. Moonshot was dropping this on import by
      // forcing `screenshot: null` and `screenshotBlocked: true`, which made
      // every imported shared Dock show the fallback boat image.
