...
export async function GET(_: Request, { params }: { params: { orgCode: string } }) {
  try {
    ...
    const { data: org, error: orgError } = await supabase.from('organizations').select('*').eq('org_code', orgCode).maybeSingle();
    if (orgError) return NextResponse.json({ error: orgError.message }, { status: 500 });
    if (!org) return NextResponse.json({ error: 'Organization not found' }, { status: 404 });
    const { data: workspace, error: wsError } = await supabase.from('workspaces').select('*').eq('organization_id', org.id).eq('status', 'published').order('published_at', { ascending: false }).order('updated_at', { ascending: false }).limit(1).maybeSingle();
    ...
    const normalizedTabs = (tabs || []).map((tab: any) => ({ title: String(tab?.title || '').trim() || 'Untitled', url: String(tab?.url || '').trim(), customIcon: String(tab?.icon_url || '').trim(), faviconUrl: '', isLocked: tab?.is_locked !== false })).filter((tab) => /^https?:\/\//i.test(tab.url));
    ...
    return NextResponse.json({ version: Number(workspace.version) || 1, type: 'dock-managed-config', organization: { id: org.id, name: org.name, orgCode: org.org_code, emailDomain: org.email_domain || '' }, license: { plan: org.plan || 'district', label: (org.plan || 'district').charAt(0).toUpperCase() + (org.plan || 'district').slice(1), maxUsers: org.max_users || 500 }, workspace: { id: workspace.id, name: workspace.name, version: Number(workspace.version) || 1, publishedAt, updatedAt, tabs: normalizedTabs }, debug: { workspaceId: workspace.id, tabCount: normalizedTabs.length } }, { headers: { 'Cache-Control': 'no-store, max-age=0', 'Access-Control-Allow-Origin': '*', 'Access-Control-Allow-Methods': 'GET, OPTIONS', 'Access-Control-Allow-Headers': '*' } });
  } catch (error: any) {
