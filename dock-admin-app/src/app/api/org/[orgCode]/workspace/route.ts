...
async function requireActiveUserForOrganization(request: NextRequest, supabase: ReturnType<typeof getServerSupabase>, org: { id: string }) {
  ...
  if (userError || !user?.id) return { error: NextResponse.json({ error: 'Invalid auth token', code: 'INVALID_AUTH_TOKEN' }, { status: 401 }) };
  ...
  if (existingError) return { error: NextResponse.json({ error: existingError.message, code: 'PROFILE_LOOKUP_FAILED' }, { status: 500 }) };
  if (!existing?.id) return { error: NextResponse.json({ error: 'User is not provisioned for this district.', code: 'PROFILE_REQUIRED' }, { status: 403 }) };
  if (normalize(existing.status || 'active').toLowerCase() !== 'active') return { error: NextResponse.json({ error: 'User account is disabled.', code: 'ACCOUNT_DISABLED' }, { status: 403 }) };
  if (normalize(existing.organization_id) !== normalize(org.id)) return { error: NextResponse.json({ error: 'User is not authorized for this district.', code: 'TENANT_MISMATCH' }, { status: 403 }) };

  const { data: allowed, error: accessError } = await supabase.rpc('dock_user_access_allowed', { p_user_id: userId, p_organization_id: org.id });
  if (accessError) return { error: NextResponse.json({ error: accessError.message, code: 'ACCESS_CHECK_FAILED' }, { status: 500 }) };
  if (!allowed) return { error: NextResponse.json({ error: 'Dock access is disabled or unauthorized.', code: 'ACCESS_DENIED' }, { status: 403 }) };

  if (userEmail && normalize(existing.email).toLowerCase() !== userEmail) {
    const { error: emailUpdateError } = await supabase.from('profiles').update({ email: userEmail, updated_at: new Date().toISOString() }).eq('id', userId).eq('organization_id', org.id);
    if (emailUpdateError) return { error: NextResponse.json({ error: emailUpdateError.message, code: 'PROFILE_EMAIL_SYNC_FAILED' }, { status: 403 }) };
  }
  return { user: { id: userId, email: userEmail, role: normalize(existing.role) || 'member' } };
}

export async function GET(request: NextRequest, { params }: { params: Promise<{ orgCode: string }> }) {
  try {
    ...
    if (licenseStatus === 'past_due') {
      ...
      if (!renewal || Date.now() > renewal + graceDays * 24 * 60 * 60 * 1000) return NextResponse.json({ error: 'District license is past due.', code: 'LICENSE_PAST_DUE' }, { status: 403 });
    }

    const { data: workspace, error: wsError } = await supabase.from('workspaces').select('id,name,version,published_at,updated_at')
      .eq('organization_id', org.id).eq('status', 'published').order('published_at', { ascending: false }).order('created_at', { ascending: false }).limit(1).maybeSingle();
    ...
    const { data: tabs, error: tabsError } = await supabase.from('workspace_tabs').select('id, title, url, icon_url, position, created_at, is_locked')
      .eq('workspace_id', workspace.id).order('position', { ascending: true }).order('created_at', { ascending: true });
    ...
    return NextResponse.json({
      ...
      license: { plan: org.plan || 'district', label: (org.plan || 'district').charAt(0).toUpperCase() + (org.plan || 'district').slice(1), maxUsers: org.max_users || 500, status: (org as any).license_status || 'trial', minimumExtensionVersion: (org as any).minimum_extension_version || null },
      workspace: {
        id: workspace.id, name: workspace.name, version: Number(workspace.version) || 1, publishedAt, updatedAt,
        branding: { districtLogoUrl: String((org as any).district_logo_url || '').trim(), districtBackgroundUrl: String((org as any).district_background_url || '').trim(), districtAccentColor: String((org as any).district_accent_color || '').trim() },
        tabs: normalizedTabs
      },
