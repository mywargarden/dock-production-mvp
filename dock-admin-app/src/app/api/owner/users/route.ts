...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const { data: profile, error: profileError } = await auth.service
      .from('profiles')
    ...
    if (!profile) return NextResponse.json({ error: 'User not found in this district.' }, { status: 404 })
    ...
    await auth.service.from('audit_logs').insert({
      ...
      action: status === 'inactive' ? 'owner_deactivate_user' : 'owner_activate_user',
      target_type: 'profile',
      target_id: userId,
      details: { email: profile.email || null, previousStatus: profile.status || null, nextStatus: status }
    }).throwOnError()

    const districts = await loadOwnerDistricts(auth.service)
    return NextResponse.json({ ok: true, districts })
  } catch (error: any) {
    console.error('Dock HQ user update failed', error)
