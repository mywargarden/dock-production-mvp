...
function hasInline(value: unknown) {
  return normalize(value).startsWith('data:image/')
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!organizationId && !orgCodeHint) {
      return NextResponse.json({ error: 'organizationId or orgCode is required.' }, { status: 400 })
    }
    ...
    const counts = { branding: 0, draftTabs: 0, liveTabs: 0, snapshots: 0, snapshotTabs: 0, snapshotBranding: 0 }
    ...
    const { data: workspaces, error: workspaceError } = await auth.service
      .from('workspaces').select('id').eq('organization_id', org.id)
    ...
    if (workspaceIds.length) {
      const { data: liveTabs, error: liveError } = await auth.service
        .from('workspace_tabs').select('id,workspace_id,icon_url,position').in('workspace_id', workspaceIds).order('position', { ascending: true })
    ...
    const { data: versions, error: versionsError } = await auth.service
      .from('workspace_versions').select('id,tabs,branding').eq('organization_id', org.id).order('created_at', { ascending: true })
    if (versionsError) throw versionsError

    for (const version of versions || []) {
      let changed = false
      const branding = version.branding && typeof version.branding === 'object' ? { ...version.branding } : {}
      if (hasInline(branding.district_logo_url)) {
        branding.district_logo_url = await materializeManagedImage(auth.service, branding.district_logo_url, { orgCode, kind: 'logo' }) || null
        counts.snapshotBranding += 1
        changed = true
      }
      if (hasInline(branding.district_background_url)) {
        branding.district_background_url = await materializeManagedImage(auth.service, branding.district_background_url, { orgCode, kind: 'background' }) || null
        counts.snapshotBranding += 1
        changed = true
      }
      ...
      if (changed) {
        const { error } = await auth.service
          ...
          .update({ tabs: materialized.tabs, branding })
          .eq('id', version.id)
