...
export default function Rc1Settings(){
  ...
  async function save(){
    setBusy(true)
    try{
      const r=await authFetch('/api/owner/settings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'save',settings})})
  ...
  return <main className="hq2Content" style={{maxWidth:1100,margin:'0 auto'}}>
    <section className="hq2Hero"><div><div className="hq2Eyebrow">Dock 1.0 RC1</div><h1>Owner Settings</h1><p>Launch defaults, support routing, and permissions that are active in Dock 1.0.</p></div></section>
    ...
    <section className="hq2Card"><h2>Defaults & support</h2><div className="hq2FormGrid">
      {[['Trial days','default_trial_days','number'],['Grace days','default_grace_days','number'],['Default seats','default_seats','number'],['Default plan','default_plan','text'],['Minimum extension version','default_minimum_version','text'],['Default theme','default_theme','text'],['Support email','support_email','email'],['Billing email','billing_email','email'],['Support URL','support_url','text']].map(([label,key,type])=><div className="hq2Field" key={key}><label>{label}</label><input type={type} value={settings[key]??''} onChange={e=>patch(key,type==='number'?Number(e.target.value):e.target.value)}/></div>)}
    </div></section>

    <section className="hq2Card"><h2>District defaults</h2>
      ...
      <label className="hq2Check"><input type="checkbox" checked={settings.allow_admin_branding===true} onChange={e=>patch('allow_admin_branding',e.target.checked)}/>allow admin branding</label>
      <p className="hq2Sub" style={{marginTop:10}}>Automated notifications and product-wide maintenance/emergency controls are intentionally not exposed in RC1 until they have a real runtime effect.</p>
    </section>
