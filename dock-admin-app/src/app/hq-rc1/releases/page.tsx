...
export default function Rc1Releases(){
  ...
  return <main className="hq2Content" style={{maxWidth:1100,margin:'0 auto'}}>
    ...
    <section className="hq2Grid2"><div className="hq2Card"><h2>Production state</h2><div className="hq2Ops"><div className="hq2Op"><strong>Current production</strong><span>{current?.version||'None recorded'}</span></div><div className="hq2Op"><strong>Deployment</strong><span>{current?.deployment_url||'—'}</span></div><div className="hq2Op"><strong>Artifact SHA-256</strong><span style={{overflowWrap:'anywhere'}}>{current?.artifact_sha256||'Not recorded'}</span></div><div className="hq2Op"><strong>Source commit</strong><span style={{overflowWrap:'anywhere'}}>{current?.source_commit||'Not recorded'}</span></div></div></div><div className="hq2Card"><h2>Version telemetry</h2>{Object.keys(versionCounts).length?Object.entries(versionCounts).map(([v,n])=><div className="hq2User" key={v}><strong>{v}</strong><span>{n} installs</span></div>):<div className="hq2Empty">No installation telemetry yet.</div>}</div></section>
    <section className="hq2Card"><h2>Prepare RC</h2><div className="hq2FormGrid">{[['Version','version'],['Deployment URL','deployment_url'],['Chrome status','chrome_status'],['Artifact SHA-256','artifact_sha256'],['Source commit SHA','source_commit'],['Source tree SHA','source_tree_sha'],['Artifact size (bytes)','artifact_size_bytes'],['Internal notes','notes_internal'],['Public notes','notes_public']].map(([label,key])=><div className={`hq2Field ${key.includes('notes')?'full':''}`} key={key}><label>{label}</label>{key.includes('notes')?<textarea value={draft[key]||''} onChange={e=>patch(key,e.target.value)}/>:<input value={draft[key]||''} onChange={e=>patch(key,e.target.value)}/>}</div>)}<div className="hq2Field"><label>Channel</label><select value={draft.channel} onChange={e=>patch('channel',e.target.value)}>{['development','pilot','beta','production'].map(x=><option key={x}>{x}</option>)}</select></div><div className="hq2Field"><label>Status</label><select value={draft.status} onChange={e=>patch('status',e.target.value)}>{['draft','preview','production','paused','retired'].map(x=><option key={x}>{x}</option>)}</select></div></div>
    ...
    <section className="hq2Card"><h2>Release records</h2>{releases.length?<table className="hq2Table"><thead><tr><th>Version</th><th>Status</th><th>Artifact</th><th>Source</th></tr></thead><tbody>{releases.map((r:any)=><tr key={r.id}><td>{r.version}</td><td>{r.status}</td><td style={{overflowWrap:'anywhere'}}>{r.artifact_sha256||'—'}</td><td style={{overflowWrap:'anywhere'}}>{r.source_commit||'—'}</td></tr>)}</tbody></table>:<div className="hq2Empty">No release records yet.</div>}</section>
