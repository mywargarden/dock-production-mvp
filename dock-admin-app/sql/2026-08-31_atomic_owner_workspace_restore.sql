-- Dock 1.0 RC1 atomic owner workspace restore.
-- Keeps snapshot capture, live workspace replacement, branding restore, and audit evidence
-- inside one database transaction so a partial failure cannot leave mixed customer state.
...
begin
  if p_organization_id is null or p_snapshot_id is null then
    raise exception 'ORGANIZATION_AND_SNAPSHOT_REQUIRED';
  end if;
  ...
  insert into public.workspace_tabs (
    workspace_id, title, url, icon_url, position, is_locked, updated_at
  )
  select
    v_live.id,
    ...
    (ord - 1)::integer,
    case
      when elem ? 'is_locked' and jsonb_typeof(elem->'is_locked') = 'boolean'
        then (elem->>'is_locked')::boolean
      else true
    end,
    v_now
  ...
  insert into public.audit_logs (
    organization_id, actor_email, action, target_type, target_id, details
  ) values (
    p_organization_id,
